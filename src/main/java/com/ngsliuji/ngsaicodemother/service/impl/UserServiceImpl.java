...
import com.ngsliuji.ngsaicodemother.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        //使用非MyBatisPlus提供的方法,传统方式，直接使用mapper
        ...
        //3.对密码进行加密
        String encryptePassword = getEncryptePassword(userPassword);
        //4.向数据库中插入数据

        User user = new User();
        user.setUserAccount(userAccount);
        user.setUserPassword(encryptePassword);
        user.setUserName("无名");
    ...
    //密码加密
    @Override
    public String getEncryptePassword(String userPassword) {
        //加盐 混淆密码
        final String SALT = "ngsliuji";
        return DigestUtils.md5DigestAsHex((SALT + userPassword).getBytes());
        //.getBytes()获取字节数据；.md5DigestAsHex()对字节数据进行md5加密，并返回加密后的字符串
    }

    @Override
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
//        注意多补充一些؜؜؜؜؜؜؜؜؜؜؜؜؜؜؜校验条件，在用户登录成功后，将用户信息存储在当前的 Session 中。
        //1.校验参数
        ...
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("userAccount", userAccount);
        queryWrapper.eq("userPassword", getEncryptePassword(userPassword));
    ...
    public List<UserVO> getUserVOList(List<User> userList) {
        ...
        //.map()方法：针对集合中的每个元素，执行一个指定的操作，并返回结果。
        //.collect()方法：将集合中的元素转换成一个新的列表。
